...
/// Describe a process based on its executable name.
/// Returns None if the process is not in the knowledge base.
pub fn describe_process(name: &str) -> Option<ProcessDescription> {
    let lower = name.to_lowercase();
    ...
    match lower.as_str() {
        ...
        "system" | "[system process]" => Some(ProcessDescription {
            ...
            description: "Nhân hệ điều hành Windows, quản lý tất cả tài nguyên phần cứng và phần mềm.".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Cực kỳ quan trọng — KHÔNG THỂ tắt".to_string(),
            can_kill: false,
        }),
        "csrss.exe" => Some(ProcessDescription {
            what: "Client/Server Runtime Subsystem".to_string(),
            description: "Process hệ thống cốt lõi, quản lý console windows và thread. Tắt sẽ gây BSOD.".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Cực kỳ quan trọng — tắt sẽ gây màn hình xanh".to_string(),
            can_kill: false,
        }),
        "lsass.exe" => Some(ProcessDescription {
            what: "Local Security Authority Subsystem".to_string(),
            description: "Xác thực đăng nhập người dùng, quản lý chính sách bảo mật và token.".to_string(),
            belongs_to: "Windows Security".to_string(),
            importance: "Cực kỳ quan trọng — tắt sẽ buộc đăng xuất/khởi động lại".to_string(),
            can_kill: false,
        }),
        "smss.exe" => Some(ProcessDescription {
            what: "Session Manager Subsystem".to_string(),
            description: "Khởi tạo session Windows, chạy csrss.exe và winlogon.exe cho mỗi session.".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Cực kỳ quan trọng — KHÔNG THỂ tắt".to_string(),
            can_kill: false,
        }),
        "wininit.exe" => Some(ProcessDescription {
            what: "Windows Initialization Process".to_string(),
            description: "Khởi động các service hệ thống cốt lõi (services.exe, lsass.exe).".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Cực kỳ quan trọng — KHÔNG THỂ tắt".to_string(),
            can_kill: false,
        }),
        "winlogon.exe" => Some(ProcessDescription {
            what: "Windows Logon Process".to_string(),
            description: "Quản lý đăng nhập/đăng xuất, xử lý Ctrl+Alt+Del, khóa màn hình.".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Cực kỳ quan trọng — tắt sẽ buộc đăng xuất".to_string(),
            can_kill: false,
        }),
        "services.exe" => Some(ProcessDescription {
            what: "Service Control Manager".to_string(),
            description: "Quản lý tất cả Windows Services. Là parent process của mọi svchost.exe.".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Cực kỳ quan trọng — KHÔNG THỂ tắt".to_string(),
            can_kill: false,
        }),
        "memory compression" => Some(ProcessDescription {
            what: "Memory Compression".to_string(),
            description: "Nén các trang bộ nhớ ít dùng để giải phóng RAM vật lý, giảm nhu cầu dùng page file.".to_string(),
            belongs_to: "Windows Memory Manager".to_string(),
            importance: "Quan trọng — giúp tối ưu RAM, không nên tắt".to_string(),
            can_kill: false,
        }),
        "registry" => Some(ProcessDescription {
            what: "Registry Process".to_string(),
            description: "Quản lý Windows Registry hives trong bộ nhớ.".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Cực kỳ quan trọng — KHÔNG THỂ tắt".to_string(),
            can_kill: false,
        }),

        // ===== Services (kill with caution) =====
        "svchost.exe" => Some(ProcessDescription {
            what: "Service Host Process".to_string(),
            description: "Process chung chứa nhiều Windows Services. Mỗi instance chạy các service khác nhau (Windows Update, DHCP, Audio...).".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Quan trọng — chứa nhiều service thiết yếu, tắt có thể gây lỗi hệ thống".to_string(),
            can_kill: false,
        }),
        "explorer.exe" => Some(ProcessDescription {
            what: "Windows Explorer".to_string(),
            description: "Desktop shell — quản lý Taskbar, Start Menu, File Explorer. Tắt sẽ mất desktop nhưng có thể khởi động lại.".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Quan trọng nhưng có thể restart — Task Manager > File > Run 'explorer.exe'".to_string(),
            can_kill: true,
        }),
        "dwm.exe" => Some(ProcessDescription {
            what: "Desktop Window Manager".to_string(),
            description: "Render giao diện đồ họa Windows (composition, transparency, animations).".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Quan trọng — tắt sẽ mất hiệu ứng đồ họa".to_string(),
            can_kill: false,
        }),
        "searchindexer.exe" | "searchhost.exe" => Some(ProcessDescription {
            what: "Windows Search Indexer".to_string(),
            description: "Lập chỉ mục file để tìm kiếm nhanh trong Start Menu và File Explorer.".to_string(),
            belongs_to: "Windows Search".to_string(),
            importance: "Không thiết yếu — có thể tắt nếu không cần search nhanh".to_string(),
            can_kill: true,
        }),
        "spoolsv.exe" => Some(ProcessDescription {
            what: "Print Spooler Service".to_string(),
            description: "Quản lý hàng đợi in. Cần thiết nếu dùng máy in.".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Không thiết yếu nếu không dùng máy in".to_string(),
            can_kill: true,
        }),
        "runtimebroker.exe" => Some(ProcessDescription {
            what: "Runtime Broker".to_string(),
            description: "Quản lý permissions cho ứng dụng UWP/Microsoft Store. Mỗi UWP app có 1 instance.".to_string(),
            belongs_to: "Windows OS".to_string(),
            importance: "Bình thường — tự khởi động lại khi cần".to_string(),
            can_kill: true,
        }),

        // ===== Browsers =====
        "chrome.exe" => Some(ProcessDescription {
            what: "Google Chrome".to_string(),
            description: "Trình duyệt web Google Chrome. Mỗi tab và extension chạy trong process riêng (multi-process architecture).".to_string(),
            belongs_to: "Google LLC".to_string(),
            importance: "Bình thường — có thể tắt, mất các tab đang mở".to_string(),
            can_kill: true,
        }),
        "msedge.exe" => Some(ProcessDescription {
            what: "Microsoft Edge".to_string(),
            description: "Trình duyệt web Microsoft Edge (Chromium-based). Multi-process giống Chrome.".to_string(),
            belongs_to: "Microsoft".to_string(),
            importance: "Bình thường — có thể tắt".to_string(),
            can_kill: true,
        }),
        "firefox.exe" => Some(ProcessDescription {
            what: "Mozilla Firefox".to_string(),
            description: "Trình duyệt web Firefox. Dùng multi-process (Electrolysis) cho tabs.".to_string(),
            belongs_to: "Mozilla Foundation".to_string(),
            importance: "Bình thường — có thể tắt".to_string(),
            can_kill: true,
        }),
        "brave.exe" => Some(ProcessDescription {
            what: "Brave Browser".to_string(),
            description: "Trình duyệt web Brave (Chromium-based) với tính năng chặn quảng cáo tích hợp.".to_string(),
            belongs_to: "Brave Software".to_string(),
            importance: "Bình thường — có thể tắt".to_string(),
            can_kill: true,
        }),

        // ===== Dev Tools =====
        "code.exe" => Some(ProcessDescription {
            what: "Visual Studio Code".to_string(),
            description: "Trình soạn thảo code VS Code. Multi-process: main, renderer, extension host, terminal.".to_string(),
            belongs_to: "Microsoft".to_string(),
            importance: "Bình thường — có thể tắt, lưu file trước".to_string(),
            can_kill: true,
        }),
        "devenv.exe" => Some(ProcessDescription {
            what: "Visual Studio IDE".to_string(),
            description: "Môi trường phát triển tích hợp Visual Studio. Dùng nhiều RAM cho IntelliSense và build.".to_string(),
            belongs_to: "Microsoft".to_string(),
            importance: "Bình thường — có thể tắt, lưu project trước".to_string(),
            can_kill: true,
        }),
        "node.exe" => Some(ProcessDescription {
            what: "Node.js Runtime".to_string(),
            description: "JavaScript runtime. Thường là dev server, build tool, hoặc backend service.".to_string(),
            belongs_to: "OpenJS Foundation".to_string(),
            importance: "Bình thường — tắt sẽ dừng dev server/script đang chạy".to_string(),
            can_kill: true,
        }),
        "python.exe" | "pythonw.exe" | "python3.exe" => Some(ProcessDescription {
            what: "Python Interpreter".to_string(),
            description: "Chạy script/ứng dụng Python. Có thể là script, web server, ML training...".to_string(),
            belongs_to: "Python Software Foundation".to_string(),
            importance: "Bình thường — tắt sẽ dừng script đang chạy".to_string(),
            can_kill: true,
        }),
        "java.exe" | "javaw.exe" => Some(ProcessDescription {
            what: "Java Virtual Machine".to_string(),
            description: "Chạy ứng dụng Java (IntelliJ IDEA, Minecraft, enterprise apps...).".to_string(),
            belongs_to: "Oracle / OpenJDK".to_string(),
            importance: "Bình thường — tắt sẽ dừng ứng dụng Java đang chạy".to_string(),
            can_kill: true,
        }),
        "cargo.exe" => Some(ProcessDescription {
            what: "Rust Package Manager".to_string(),
            description: "Build tool và package manager cho Rust. Thường đang build/compile project.".to_string(),
            belongs_to: "Rust Project".to_string(),
            importance: "Bình thường — tắt sẽ dừng quá trình build".to_string(),
            can_kill: true,
        }),
        "git.exe" => Some(ProcessDescription {
            what: "Git Version Control".to_string(),
            description: "Hệ thống quản lý phiên bản mã nguồn. Đang thực hiện git operation.".to_string(),
            belongs_to: "Git Project".to_string(),
            importance: "Bình thường — tắt có thể gây corrupt index nếu đang write".to_string(),
            can_kill: true,
        }),

        // ===== VM/WSL/Docker =====
        "vmmem" | "vmmemwsl" => Some(ProcessDescription {
            what: "Virtual Machine Memory".to_string(),
            description: "Đại diện bộ nhớ của WSL2/Hyper-V VM. Kích thước = RAM VM đang dùng. Giảm bằng cách tắt WSL: 'wsl --shutdown'.".to_string(),
            belongs_to: "WSL2 / Hyper-V".to_string(),
            importance: "Quan trọng nếu đang dùng WSL — 'wsl --shutdown' để giải phóng RAM".to_string(),
            can_kill: false,
        }),
        "vmwp.exe" => Some(ProcessDescription {
            what: "Hyper-V VM Worker Process".to_string(),
            description: "Worker process cho mỗi Hyper-V virtual machine đang chạy.".to_string(),
            belongs_to: "Microsoft Hyper-V".to_string(),
            importance: "Quan trọng — tắt sẽ force-stop VM".to_string(),
            can_kill: true,
        }),
        "vmms.exe" => Some(ProcessDescription {
            what: "Hyper-V Virtual Machine Management".to_string(),
            description: "Service quản lý tất cả Hyper-V VMs.".to_string(),
            belongs_to: "Microsoft Hyper-V".to_string(),
            importance: "Quan trọng — cần thiết cho Hyper-V/WSL2".to_string(),
            can_kill: false,
        }),
        "wslhost.exe" | "wsl.exe" | "wslservice.exe" => Some(ProcessDescription {
            what: "Windows Subsystem for Linux".to_string(),
            description: "Host process cho WSL. Quản lý Linux distributions và inter-op với Windows.".to_string(),
            belongs_to: "Microsoft WSL".to_string(),
            importance: "Cần thiết cho WSL — 'wsl --shutdown' để tắt đúng cách".to_string(),
            can_kill: true,
        }),
        "com.docker.backend.exe" | "dockerd.exe" | "docker.exe" => Some(ProcessDescription {
            what: "Docker Engine".to_string(),
            description: "Docker container runtime. Quản lý và chạy containers.".to_string(),
            belongs_to: "Docker Inc.".to_string(),
            importance: "Cần thiết nếu đang dùng containers — tắt qua Docker Desktop".to_string(),
            can_kill: true,
        }),

        // ===== Common Apps =====
        "discord.exe" => Some(ProcessDescription {
            what: "Discord".to_string(),
            description: "Ứng dụng chat/voice Discord (Electron-based, multi-process).".to_string(),
            belongs_to: "Discord Inc.".to_string(),
            importance: "Bình thường — có thể tắt".to_string(),
            can_kill: true,
        }),
        "spotify.exe" => Some(ProcessDescription {
            what: "Spotify".to_string(),
            description: "Ứng dụng nghe nhạc Spotify.".to_string(),
            belongs_to: "Spotify AB".to_string(),
            importance: "Bình thường — có thể tắt".to_string(),
            can_kill: true,
        }),
        "slack.exe" => Some(ProcessDescription {
            what: "Slack".to_string(),
            description: "Ứng dụng nhắn tin Slack cho team (Electron-based).".to_string(),
            belongs_to: "Salesforce / Slack".to_string(),
            importance: "Bình thường — có thể tắt".to_string(),
            can_kill: true,
        }),
        "teams.exe" | "ms-teams.exe" => Some(ProcessDescription {
            what: "Microsoft Teams".to_string(),
            description: "Ứng dụng họp và chat Microsoft Teams.".to_string(),
            belongs_to: "Microsoft".to_string(),
            importance: "Bình thường — có thể tắt khi không họp".to_string(),
            can_kill: true,
        }),
        "telegram.exe" => Some(ProcessDescription {
            what: "Telegram Desktop".to_string(),
            description: "Ứng dụng nhắn tin Telegram.".to_string(),
            belongs_to: "Telegram FZ-LLC".to_string(),
            importance: "Bình thường — có thể tắt".to_string(),
            can_kill: true,
        }),
        "steam.exe" | "steamwebhelper.exe" => Some(ProcessDescription {
            what: "Steam".to_string(),
            description: "Nền tảng game Steam. steamwebhelper.exe là trình duyệt tích hợp.".to_string(),
            belongs_to: "Valve Corporation".to_string(),
            importance: "Bình thường — có thể tắt khi không chơi game".to_string(),
            can_kill: true,
        }),

        // ===== Antivirus =====
        "msmpeng.exe" => Some(ProcessDescription {
            what: "Windows Defender Antimalware".to_string(),
            description: "Engine quét virus/malware của Windows Defender. Có thể dùng nhiều CPU khi quét.".to_string(),
            belongs_to: "Microsoft Defender".to_string(),
            importance: "Quan trọng — bảo vệ hệ thống khỏi malware".to_string(),
            can_kill: false,
        }),
        "securityhealthservice.exe" => Some(ProcessDescription {
            what: "Windows Security Health Service".to_string(),
            description: "Giám sát tình trạng bảo mật tổng thể của Windows.".to_string(),
            belongs_to: "Windows Security".to_string(),
            importance: "Quan trọng — theo dõi bảo mật hệ thống".to_string(),
            can_kill: false,
        }),
